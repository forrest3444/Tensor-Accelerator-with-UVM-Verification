...
`ifdef ASSERT_ON
  initial begin
    assert ((A_SPAD_BASE0 + SPAD_BUFFER_BYTES) <= A_SPAD_BASE1)
      else $fatal(1, "A fixed scratchpad windows overlap");
    assert ((A_SPAD_BASE1 + SPAD_BUFFER_BYTES) <= B_SPAD_BASE0)
      else $fatal(1, "A/B fixed scratchpad windows overlap");
    assert ((B_SPAD_BASE0 + SPAD_BUFFER_BYTES) <= BIAS_SPAD_BASE)
      else $fatal(1, "B/bias fixed scratchpad windows overlap");
    assert ((BIAS_SPAD_BASE + SPAD_BUFFER_BYTES) <= SPAD_BYTES)
      else $fatal(1, "Fixed scratchpad windows exceed SPAD capacity");
  end
`endif
